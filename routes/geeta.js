...
const GeetaCategory = require('../models/GeetaCategory');
const GeetaName = require('../models/GeetaName');
const GeetaChapter = require('../models/GeetaChapter');
const GeetaContent = require('../models/GeetaContent');

// Configure multer for file uploads
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const uploadDir = 'public/uploads/geeta';
...
router.get('/', async (req, res) => {
    try {
        const categories = await GeetaCategory.find().sort({ createdAt: -1 });
        res.render('geeta/index', { 
            categories,
            activePage: 'geeta',
...
router.get('/category', async (req, res) => {
    try {
        const categories = await GeetaCategory.find().sort({ createdAt: -1 });
        res.render('geeta/category/index', { 
            categories,
            activePage: 'geeta',
...
router.post('/category/add', upload.single('cover_image'), async (req, res) => {
    try {
        ...
        if (req.file) {
            cover_image = '/uploads/geeta/' + req.file.filename;
        }

        const category = new GeetaCategory({
            name,
            cover_image
        });
...
router.get('/category/edit/:id', async (req, res) => {
    try {
        const category = await GeetaCategory.findById(req.params.id);
        if (!category) {
            return res.status(404).send('Category not found');
        }
        res.render('geeta/category/edit', {
            category,
            activePage: 'geeta',
            activeSection: 'category',
            returnPath: req.query.return || '/geeta'
        });
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error');
...
router.post('/category/edit/:id', upload.single('cover_image'), async (req, res) => {
    try {
        const { name } = req.body;
        const returnPath = req.body.return || '/geeta';
        const updateData = { name };
        
        if (req.file) {
            updateData.cover_image = '/uploads/geeta/' + req.file.filename;
        }

        const category = await GeetaCategory.findByIdAndUpdate(
            req.params.id,
            updateData,
            { new: true }
        );
...
router.post('/category/delete/:id', async (req, res) => {
    try {
        ...
        const hasBooks = await GeetaName.exists({ category: req.params.id });
        if (hasBooks) {
...
router.get('/name', async (req, res) => {
    try {
        const books = await GeetaName.find()
            .populate('category')
            .sort({ createdAt: -1 });
        const categories = await GeetaCategory.find();
        res.render('geeta/name/index', { 
            books,
            categories,
            activePage: 'geeta',
            activeSection: 'name',
...
router.get('/name/add', async (req, res) => {
    try {
        const categories = await GeetaCategory.find().sort({ name: 1 });
        const selectedCategoryId = req.query.category || null;
        res.render('geeta/name/add', {
            categories,
            selectedCategoryId,
            activePage: 'geeta',
            activeSection: 'name'
...
router.post('/name/add', upload.single('book_image'), async (req, res) => {
    try {
        ...
        if (req.file) {
            book_image = '/uploads/geeta/' + req.file.filename;
        }

        const book = new GeetaName({
            name,
            category,
            book_image
        });
...
router.get('/name/edit/:id', async (req, res) => {
    try {
        const book = await GeetaName.findById(req.params.id).populate('category');
        const categories = await GeetaCategory.find().sort({ name: 1 });
        ...
        res.render('geeta/name/edit', {
            book,
            categories,
            activePage: 'geeta',
            activeSection: 'name',
            returnPath: req.query.return || '/geeta/name'
        });
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error');
...
router.post('/name/edit/:id', upload.single('book_image'), async (req, res) => {
    try {
        const { name, category } = req.body;
        const returnPath = req.body.return || '/geeta/name';
        const updateData = { name, category };
        
        if (req.file) {
            updateData.book_image = '/uploads/geeta/' + req.file.filename;
        }

        const book = await GeetaName.findByIdAndUpdate(
            req.params.id,
            updateData,
            { new: true }
        );
...
router.post('/name/delete/:id', async (req, res) => {
    try {
        const book = await GeetaName.findById(req.params.id);
        const returnPath = req.body.return || '/geeta/name';
        ...
        const hasChapters = await GeetaChapter.exists({ book: req.params.id });
        if (hasChapters) {
...
router.get('/chapter', async (req, res) => {
    try {
        ...
        const categories = await GeetaCategory.find();
        const books = await GeetaName.find();
        res.render('geeta/chapter/index', { 
            chapters,
            categories,
            books,
            activePage: 'geeta',
...
router.get('/chapter/add', async (req, res) => {
    try {
        const categories = await GeetaCategory.find().sort({ name: 1 });
        const books = await GeetaName.find().sort({ name: 1 });
        const selectedCategoryId = req.query.category || null;
        const selectedBookId = req.query.book || null;
        res.render('geeta/chapter/add', {
            categories,
            books,
            selectedCategoryId,
            selectedBookId,
...
router.post('/chapter/add', async (req, res) => {
    try {
        ...
        const chapter = new GeetaChapter({
            name,
            category,
            book
        });
        ...
        if (req.body.book && req.body.category) {
            res.redirect(`/geeta/category/${req.body.category}/book/${req.body.book}`);
        } else {
            res.redirect('/geeta/chapter');
        }
    } catch (err) {
        console.error(err);
...
router.get('/chapter/edit/:id', async (req, res) => {
    try {
        const chapter = await GeetaChapter.findById(req.params.id)
            .populate('category')
            .populate('book');
        const categories = await GeetaCategory.find().sort({ name: 1 });
        const books = await GeetaName.find().sort({ name: 1 });
        ...
        res.render('geeta/chapter/edit', {
            chapter,
            categories,
            books,
            activePage: 'geeta',
            activeSection: 'chapter',
            returnPath: req.query.return || '/geeta/chapter'
        });
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error');
...
router.post('/chapter/edit/:id', async (req, res) => {
    try {
        const { name, category, book } = req.body;
        const returnPath = req.body.return || '/geeta/chapter';
        const chapter = await GeetaChapter.findByIdAndUpdate(
            req.params.id,
            { name, category, book },
            { new: true }
        );
...
router.post('/chapter/delete/:id', async (req, res) => {
    try {
        ...
        const hasContent = await GeetaContent.exists({ chapter: req.params.id });
        if (hasContent) {
...
router.get('/chapter/:chapterId/contents', async (req, res) => {
    try {
        const contents = await GeetaContent.find({ chapter: req.params.chapterId })
            .populate('category')
            .populate('book')
            .populate('chapter')
            .sort({ sequence: 1 });
        const chapter = await GeetaChapter.findById(req.params.chapterId)
            .populate('category')
            .populate('book');
        if (!chapter) {
            return res.status(404).send('Chapter not found');
...
router.get('/content', async (req, res) => {
    try {
        const contents = await GeetaContent.find()
            .populate('category')
            .populate('book')
            .populate('chapter')
            .sort({ sequence: 1 });
        ...
        const books = await GeetaName.find();
        const chapters = await GeetaChapter.find();
        res.render('geeta/content/index', { 
            contents,
            categories,
            books,
            chapters,
    ...
    try {
        ...
        if (categoryId && bookId && chapterId) {
            ...
            const [category, book, chapter] = await Promise.all([
                GeetaCategory.findById(categoryId),
                GeetaName.findById(bookId),
                GeetaChapter.findById(chapterId)
            ]);
            ...
            if (category && book && chapter) {
                ...
                return res.render('geeta/content/add-simple', { 
                    category,
                    book,
                    chapter,
                    activePage: 'geeta',
        ...
        const categories = await GeetaCategory.find().sort({ name: 1 });
        const books = await GeetaName.find().populate('category', '_id name').sort({ name: 1 });
        const chapters = await GeetaChapter.find().populate('book', '_id name').sort({ name: 1 });
            
        console.log('>>> RENDERING add.ejs (full form) <<<');
        res.render('geeta/content/add', { 
            categories, 
            books, 
            chapters,
...
router.post('/content/add', upload.array('images', 10), async (req, res) => {
    try {
        ...
        const savedChapter = await GeetaChapter.findById(chapter).populate('book');
        if (savedChapter && savedChapter.book) {
            const bookCategory = await GeetaName.findById(savedChapter.book._id).populate('category');
            if (bookCategory && bookCategory.category) {
                res.redirect(`/geeta/category/${bookCategory.category._id}/book/${bookCategory._id}/chapter/${chapter}`);
                return;
            }
        }
...
router.get('/content/edit/:id', async (req, res) => {
    try {
        const content = await GeetaContent.findById(req.params.id);
        const categories = await GeetaCategory.find().sort({ name: 1 });
        const books = await GeetaName.find()
...
router.post('/content/edit/:id', upload.array('images', 10), async (req, res) => {
    try {
        ...
        const content = await GeetaContent.findByIdAndUpdate(
            req.params.id,
            updateData,
            { new: true }
        );
        ...
        const updatedContent = await GeetaContent.findById(req.params.id).populate('chapter');
        if (updatedContent && updatedContent.chapter) {
            const chapter = await GeetaChapter.findById(updatedContent.chapter._id).populate('book');
            if (chapter && chapter.book) {
                const bookCategory = await GeetaName.findById(chapter.book._id).populate('category');
                if (bookCategory && bookCategory.category) {
                    res.redirect(`/geeta/category/${bookCategory.category._id}/book/${bookCategory._id}/chapter/${updatedContent.chapter._id}`);
                    return;
                }
            }
        }
...
router.get('/chapter/export-excel', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="geeta_chapters.xlsx"');
        res.send(buffer);
    } catch (error) {
...
router.post('/chapter/upload-excel', upload.single('excelFile'), async (req, res) => {
    try {
        if (!req.file) {
            return res.render('geeta/chapter/index', {
                chapters: await GeetaChapter.find().populate('category').populate('book').sort({ createdAt: -1 }),
                categories: await GeetaCategory.find(),
                books: await GeetaName.find(),
                activePage: 'geeta',
                activeSection: 'chapter',
                activeTab: 'chapter',
        ...
        for (const row of data) {
            ...
            const category = await GeetaCategory.findOne({ name: row.Category });
            const book = await GeetaName.findOne({ name: row.Book });
...
router.post('/content/upload-excel', upload.single('excelFile'), async (req, res) => {
    try {
        ...
        if (!req.file) {
            ...
            if (hasContext) {
                return res.redirect(`/geeta/category/${categoryId}/book/${bookId}/chapter/${chapterId}?error=Please upload an Excel file.`);
            }
            return res.render('geeta/content/index', {
                contents: await GeetaContent.find().populate('category').populate('book').populate('chapter').sort({ sequence: 1 }),
                categories: await GeetaCategory.find(),
                books: await GeetaName.find(),
                chapters: await GeetaChapter.find(),
                activePage: 'geeta',
                activeSection: 'content',
                activeTab: 'content',
                error: 'Please upload an Excel file.'
        ...
        if (data.length === 0) {
            fs.unlinkSync(req.file.path);
            if (hasContext) {
                return res.redirect(`/geeta/category/${categoryId}/book/${bookId}/chapter/${chapterId}?error=Excel file is empty or has no data rows.`);
            }
            return res.render('geeta/content/index', {
                contents: await GeetaContent.find().populate('category').populate('book').populate('chapter').sort({ sequence: 1 }),
                categories: await GeetaCategory.find(),
        ...
        if (hasContext && chapterId) {
            const maxContent = await GeetaContent.findOne({ chapter: chapterId }).sort({ sequence: -1 });
        ...
        if (hasContext) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                const category = await GeetaCategory.findOne({ name: row.Category });
                const book = await GeetaName.findOne({ name: row.Book });
                const chapter = await GeetaChapter.findOne({ name: row.Chapter });
                ...
                if (!chapterSequences[chapter._id.toString()]) {
                    const chapterMaxContent = await GeetaContent.findOne({ chapter: chapter._id }).sort({ sequence: -1 });
        ...
        if (contents.length > 0) {
            await GeetaContent.insertMany(contents);
            ...
            if (chapterId && categoryId && bookId) {
                ...
                return res.redirect(`/geeta/category/${categoryId}/book/${bookId}/chapter/${chapterId}?success=${message}`);
            }
            ...
            if (chapterId && categoryId && bookId) {
                ...
                return res.redirect(`/geeta/category/${categoryId}/book/${bookId}/chapter/${chapterId}?error=${errorMsg}`);
            }
        ...
        res.render('geeta/content/index', {
            contents: await GeetaContent.find().populate('category').populate('book').populate('chapter').sort({ createdAt: -1 }),
            categories: await GeetaCategory.find(),
            books: await GeetaName.find(),
            chapters: await GeetaChapter.find(),
            activePage: 'geeta',
...
router.get('/content/export-excel', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', 'attachment; filename="geeta_content_upload_template.xlsx"');
...
router.get('/debug/geetas', async (req, res) => {
    try {
        const categories = await GeetaCategory.find();
        const books = await GeetaName.find().populate('category');
        const chapters = await GeetaChapter.find().populate('category').populate('book');
...
router.get('/category/:categoryId', async (req, res) => {
    try {
        ...
        const category = await GeetaCategory.findById(categoryId);
        if (!category) {
        ...
        const books = await GeetaName.find({ category: category._id })
            .populate('category')
            .sort({ createdAt: -1 });
...
router.get('/category/:categoryId/book/:bookId', async (req, res) => {
    try {
        const category = await GeetaCategory.findById(req.params.categoryId);
        const book = await GeetaName.findById(req.params.bookId).populate('category');
        ...
        const chapters = await GeetaChapter.find({ book: req.params.bookId })
            .populate('book')
            .populate('category')
            .sort({ createdAt: -1 });
        
        res.render('geeta/book/chapters', {
            category,
            book,
            chapters,
            activePage: 'geeta',
